async function keksall(a, b, eng) {
    ...
    if (timerTrittall !== undefined && cd - (Date.now() - timerTrittall) > 0) {
        ...
        let txttritt = `╔══✪〘 *KEKSE FÜR ALLE* 〙✪══\n`
        for (let i = 0; i < groupMemtritt.length; i++) {
            txttritt += '╠➥'
            txttritt += `@${a.sender.id.replace(/@c.us/g, '')} gibt @${groupMemtritt[i].id.replace(/@c.us/g, '')} einen Keks🍪\n`
        }
...
const helpobj = {
    ...
    'usage': `keksall`,
    'permission': 'foruser',
    'description': 'Gib jedem aus der Gruppe einen Keks.'
};
